...
Сделать кнопку "Add view", при нажатии на которую добавляется еще одно представление списков (располагается под исходным). В этом представлении списки второго уровня меняются на однострочные списки, в которых элементы расположены через запятую. Обращения на сервер при этом происходить не должно.
